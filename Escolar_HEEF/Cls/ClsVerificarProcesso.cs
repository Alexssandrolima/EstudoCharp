    ...
    class ClsVerificarProcesso
    {
        public void fecharprocesso()
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Programa " + NomeProcesso + ", esta aberto em outra sessão. ", NomeProcesso + " sendo executado", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Process.GetCurrentProcess().Kill();
                }
            }
            catch (Exception erroreexecutar)
            {
                MessageBox.Show(erroreexecutar + "\r\n Comando nao executado ", "Error ao Processa a procura de Arquivos ");
            }
            return;
        }
